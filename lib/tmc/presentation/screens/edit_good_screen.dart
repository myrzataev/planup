...
class _CreateTmcScreenState extends State<EditTmcScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xffD5E2F2),
        title: const Text("Редактировать материал"),
      ),
      body: Column(
        children: [
                            ...
                            child: DropdownButtonFormField(
                              ...
                              onChanged: (value) {
                                setState(() {
                                  ...
                                  urlRoute = value;
                                  BlocProvider.of<GetManufacturesListBloc>(
                                          context)
                                      .add(GetManufacturesListEvent(
                                          urlRoute:
                                              "${value}_manufactures/?skip=0&limit=5000000"));
                                  BlocProvider.of<GetModelsListBloc>(context)
                                      .add(GetModelsListEvent(
                                          urlRoute:
                            ...
                            if (state is EditGoodLoading) {
                              ...
                              GoRouter.of(context).pushReplacementNamed(
                                  ...
                                  queryParameters: {
                                    ...
                                    "manufacture": state
                                            .model.product?.manufacture?.name ??
                                        "",
                                    "model":
                                        state.model.product?.model?.name ?? "",
                                    "cost":
                                        state.model.product?.cost.toString(),
                                    "barcode": state.model.barcode ?? "",
                                    "nazvanieID":
                                        state.model.nazvanieId?.toString() ??
                                            "",
                                    "deleted":
                                        state.model.deleted?.toString() ?? "",
                                    "goodStatus":
