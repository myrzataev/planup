...
class _OpenServiceState extends State<OpenService> {
  ...
  Future<void> _loadWorkOrders() async {
    ...
    try {
      final uri = Uri.parse('http://planup.skynet.kg:8000/planup/locations_naryds/');
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          typeDealList = List<String>.from(Set<String>.from(workOrders.map((work) => work.dynamicFields['type_deal'])));
        });
      } else {
  ...
  void filterWorkOrdersByTypeDeal(String? typeDeal) {
    setState(() {
      if (typeDeal == null || typeDeal.isEmpty) {
        ...
        workOrders = allWorkOrders.where((workOrder) => workOrder.dynamicFields['type_deal'] == typeDeal).toList();
      }
    });
  }
                  ...
                  child: ListTile(
                    ...
                    onTap: () {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => Getopenservice(workOrders: workOrder),
                        ),
                      );
                    },
                  ),
