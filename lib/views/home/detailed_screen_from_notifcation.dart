...
class GetopenserviceFromNotification extends StatefulWidget {
  // final WorkOrder workOrders;
  final String workId;

  GetopenserviceFromNotification({required this.workId});

  @override
  _GetopenserviceState createState() => _GetopenserviceState();
}

class _GetopenserviceState extends State<GetopenserviceFromNotification> {
  ...
  Future<WorkOrder> _getOneNaryad() async {
   ...
   Future<void> _go() async {
    ...
    try {
      ...
      final headers = {'Content-Type': 'application/x-www-form-urlencoded'};
      final body = {'worker_id': '$username', 'naryd_id': '${widget.workId}'};
      final response = await http.post(uri, headers: headers, body: body);
      if (response.statusCode == 200) {
  ...
  void initState() {
    ...
    // statusWorkId =
    //     workOrdersList.first.dynamicFields['status_work_id'].toString() ?? "";
  }

  Future<void> _setupState() async {
    try {
      await _getOneNaryad(); // Wait for _go() to complete
      // Now you can safely access workOrdersList and its elements
      statusWorkId =
          workOrdersList.first.dynamicFields['status_work_id'].toString() ?? "";
      setState(() {
        isLoading = false;
      });
      // Setup other widgets that depend on workOrdersList
      // Call other functions that depend on workOrdersList
...
// Использование функции в map
      detailsWidgets = (workOrdersList.first.dynamicFields['work_fields']
        ...
        return ListTile(
          ...
          subtitle: Text(
              (hasLink || hasSchema) ? 'Показать' : displayValue.toString()),
          onTap: () {
            if (hasLink) {
              final match =
      ...
      final workDetailsMap = workOrdersList.first.dynamicFields['work_fields']
          as Map<String, dynamic>;
      ...
      // Handle errors as needed, such as showing an error message
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return workOrdersList.isNotEmpty
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          Container(
                            ...
                            child: Text(
                              '$statusInfo  №${workOrdersList.first.dynamicFields['id']} ',
                              style: TextStyle(
                                color: Colors.white,
                                fontSize:
                                    15, // Уменьшенный размер текста для статуса
                              ),
                            ),
                          ),
                        ],
